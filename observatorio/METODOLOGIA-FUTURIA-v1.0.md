...
> **Nota de compatibilidad:** el ejemplo canónico (cred 4, corr 3, nov 4, vel 3, alc 4, prof 5, conv 5, irrev 3) da **IES = 72** y **PS = 90**. El PS es idéntico al de v0.9 (la fórmula no cambió). El IRS queda deprecado; el motor debe emitir `WARNING: IRS deprecado en v1.0` si se invoca.
>
> *Erratum:* versiones previas del manual decían IES = 70. Aplicando la fórmula literal (`Σ(puntaje/5 × peso)`: 24+12+12+12+12 = 72) el resultado correcto es **72**. El motor `motor_calificacion.py` valida contra 72 en su `--self-test`.
...
3. SAT: ninguna afirmaciones fácticas sin fuente verificable con URL, tier y fecha.
...
6. Ante evidencia insuficiente decís "no hay señal suficiente". Nunca rellanás.
